...
Implement a serverless hackathon scoring dashboard using Azure Static Web Apps (Standard) with managed Functions API and Azure Table Storage. The architecture deploys across two regions: `westeurope` for SWA and `swedencentral` for Storage, with optional Application Insights + Log Analytics for observability.
...
- storageLocation: string = 'swedencentral'
...
```yaml
- name: logName
- location: storageLocation (swedencentral)
- sku: PerGB2018
...
- Table Service + 3 Tables (Teams, Scores, Awards)
...
| 2 | storage.bicep | Storage Account + 3 Tables | Account created, tables accessible, shared key disabled |
...
| Static Web App | Authentication | GitHub OAuth via SWA built-in auth |
